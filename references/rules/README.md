---
title: "Rules — Referências"
category: "Reference"
tags: [reference, rules]
---

# Rules

Sistemas de jogo, mecânicas e guias para campanhas.

## Conteúdo

| Categoria | Arquivos | Descrição |
|-----------|----------|-----------|
| **Chronicle System** | `Sword Chronicle - Core/` | Ruleset completo do Chronicle System (Green Ronin) — sistema-alvo da migração de Aurelia |
| **Chronicle System** | `Sword Chronicle - Sorcery/` | Chronicle of Sorcery (2014) — o sistema de magia completo, com Lores modulares |
| **Homebrew** | `Sword Chronicle - Homebrew/` | Material autoral de Aurelia: Lores, Ancestries, Qualities |
| **DMG** | DMG-02-07, DMG-03, DMG-04, DMG-07, DMG-08 | Dungeon Master's Guide |
| **PHB** | PHB-01-10 | Player's Handbook |
| **Guide** | Epic Campaigns, HTGHM | Guias de criação de campanha |
| **Format** | 5-room-dungeon | Estruturas e templates |
| **Worldbuilding** | Medieval-Population | Ferramentas de worldbuilding |

### DMG-04: Creating Adventures (2024)

Capítulo 4 do DMG 2024 — guia completo para criar aventuras:

| Arquivo | Conteúdo |
|---------|----------|
| DMG-04-01 | Step-by-Step Adventures, Using Published Adventures |
| DMG-04-02 | Lay Out the Premise (premise, conflict, situations by level, setting, maps) |
| DMG-04-03 | Draw In the Players (hooks, patrons, supernatural/happenstance) |
| DMG-04-04 | Plan Encounters (objectives, combat, exploration, social, monster behavior) |
| DMG-04-05 | Bring It to an End (climax, denouement) |
| DMG-04-06 | Adventure Rewards (treasure, hoards, quest rewards) |
| DMG-04-07 | Adventure Examples (The Fouled Stream, Miner Difficulties, The Winged God, Horns of the Beast, Boreal Ball) |

## Sword Chronicle - Core

Ruleset do **Chronicle System** passado a limpo a partir de *Sword Chronicle: Feudal Fantasy Roleplaying* (Green Ronin, 2020) — 18 arquivos cobrindo motor de regras, criação de personagem, casa nobre, feitiçaria, equipamento, intriga, combate, guerra, o Narrador e o setting Shattered Era.

Índice completo e notas de migração em `Sword Chronicle - Core/README.md`.

## Sword Chronicle - Sorcery

**Chronicle of Sorcery** (Green Ronin, 2014) — a versão completa do sistema de magia, que o livro básico de 2020 resumiu no seu Cap. 6. Traz a camada **Lore** (traditions mágicas com abilities, preço e resonances próprios), seis Lores de exemplo, as três Arts comuns detalhadas e os Sorcerous Wealth Holdings.

As duas versões divergem — ver `Sword Chronicle - Sorcery/99-divergencias-2014-2020.md`.

## Sword Chronicle - Homebrew

Material autoral de Aurelia construído sobre o Chronicle System — organizado em `Lores/`, `Ancestries/` e `Qualities/`.

Primeira Lore escrita: **A Consonância**, magia tonal e bárdica ancorada na Ordem Harmônica, com a **Art of Glamour** derivada do College of Glamour e do Feyglade Veil.

## Epic Campaigns (Guy Sclanders)

Guia completo em `The-Complete-Guide-to-Creating-Epic-Campaigns/`:

- 00-introduction → 12-ending
- Master Plot, Nemesis, Plotting, NPCs, Seeding, etc.
